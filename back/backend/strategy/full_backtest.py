...
from backend.strategy.signals import (
    generate_signals, should_buy, should_sell,
...
def _run_sync(
    ...
    # ── 1. Pre-load & pre-compute indicators ───────────────────────────────────
    stocks     = get_top100_stocks()
    stock_data: dict[str, tuple[str, pd.DataFrame]] = {}

    for s in stocks[:max_candidates * 2]:
        ticker = s["ticker"]
        df = get_stock_history(ticker, months * 31 + buffer)
        if df.empty or len(df) < 60:
            continue
        df = calculate_indicators(df)
        df = generate_signals(df)
        stock_data[ticker] = (s.get("name", ticker), df)
    ...
    for day in trading_days:
        ...
        for ticker in list(positions.keys()):
            ...
            if day_low <= pos["stop_loss"]:
                ...
                # ── 與 auto_trade / scanner 相同的 should_sell 規則 ───────────
                # 取今日含前 2 天共最多 3 根做判斷（與 morning_scan 一致）
